...
| `diag_pathstyle_dotted.lt`    | Diagrams: links             | `@Link` with `pathstyle { dotted }`                    | strict    | Swap rationale: the originally-proposed `diag_arrowstyle_curvedopen` duplicates the curvedopen row already in `diag_arrowstyle_gallery.lt`, so this fills the pathstyle gap (dotted being a separate `ldiagdotted` proc from the existing solid / curvedsolid / dashed coverage). |
...
| `eq_braces_brackets.lt`       | Equations: large fences     | Single expression nesting all three fence shapes (parens, square brackets, curly braces) via `left ... right ...`; companion display nests fences around stacked fractions to force vertical growth | strict    | Companion to `eq_braced_systems` (piecewise-system) and `eq_matrix` (single-shape); fills the multi-shape, single-line gap. |
...
| `graph_scatter_plot.lt`       | Graphs                      | `@Graph` `@Data` with `points { filledcircle }` and `pairs { none }` -- pure scatter, markers only, no connecting line | strict    | Fills the points-without-pairs gap (no existing snippet exercises the path where `@Graph` emits per-point glyphs but no stroke between them). |
...
| `table_with_math.lt`          | Tables                      | `@Tbl` whose cells contain inline `@Eq` formulas (derivatives reference: powers, sin/cos, exp, 1/x) | strict    | First snippet to nest `@Eq` inside a `@Tbl` cell; routes math through two stacked galleys and pins their baseline-alignment / rule interaction. |
...
| `text_underline_word.lt`      | --                          | `@Underline` applied to a single word inside a flowing sentence | strict    | First snippet to exercise the underline-rule emission path (drawn as a horizontal line beneath the run in the current text-fill colour, separate from the bold / italic / oblique pipeline already covered by `text_styles`). |
