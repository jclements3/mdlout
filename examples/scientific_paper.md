...
abstract: |
  ...
  design and reproduce, in tabular form, error figures that match standard
  reference values to all reported digits.
---
...
rule and Simpson's 1/3 rule [@davis1984]. They form the bedrock of every
introductory numerical-analysis course and remain in routine use as the inner kernels of
adaptive quadrature routines such as QUADPACK [@piessens1983]. Despite their age, the
...
We evaluate both rules on the four reference integrals in Table @tab:integrals.
Integrals I_1 and I_2 are smooth on the closed interval and serve to confirm
the asymptotic rates; I_3 has an endpoint singularity in the first derivative,
...
Kahan compensated form [@kahan1965] to reduce floating-point cancellation
at large $n$. Absolute errors were computed against the symbolic exact
values listed above, with cross-checks against the worked examples in
Dahlquist and Bjorck [@dahlquist2008].

# Results

## Smooth integrands: confirmation of the asymptotic rates

Table @tab:errors reports the absolute error of the two rules applied to
I_1, the exponential, at six values of $n$ doubling from 4 to 128. As
predicted, the trapezoidal-rule errors fall by a factor of approximately
four under each halving of $h$, and the Simpson's-rule errors fall by
...
The convergence is illustrated graphically in Figure @fig:trapezoid, a
log-log plot of absolute error against $n$ for both rules on I_1. The
slopes are $-2.00$ and $-4.00$ respectively, in close agreement with the
Euler-Maclaurin asymptotic analysis.

![Log-log error decay versus $n$ for the trapezoidal and Simpson's rules applied to $I_1 = \int_0^1 e^x\, dx$; see Table @tab:errors for the underlying numbers.](trapezoid_convergence.svg){#fig:trapezoid}

## Integrands with endpoint singularities

Table @tab:singular summarises the absolute errors of both rules on the
singular integrals I_3 and I_4 at $n = 32, 128, 512$. Here neither rule
attains its textbook asymptotic rate: I_3, with its $\sqrt{x}$ endpoint
behaviour, shows roughly $O(n^{-3/2})$ convergence for both rules, while
I_4's $x \log x$ behaviour leaves a tail of $O(n^{-2})$ for trapezoidal
and only marginally better for Simpson.
...
rule, and the asymptotic advantage is dramatic. Our Table @tab:errors shows
roughly *six orders of magnitude* of error reduction for the same number of
nodes.
...
Third, adaptive quadrature routines such as those in QUADPACK [@piessens1983] interleave
...
designing or selecting a numerical-integration library. (The tables in this
paper were typeset with the Lout document-formatting system [@kingston1993],
which we mention only because it is the engine that powers mdlout, the
toolchain used to produce this document.)
...
[@piessens1983]: R. Piessens, E. de Doncker-Kapenga, C. W. Uberhuber, and D. K. Kahaner. *QUADPACK: A Subroutine Package for Automatic Integration.* Springer-Verlag, Berlin, 1983.
...
[@dahlquist2008]: G. Dahlquist and A. Bjorck. *Numerical Methods in Scientific Computing*, volume 1. SIAM, Philadelphia, 2008.
