...
| [`marginalia.md`](marginalia.md) | doc | 2 | A widened right margin (5.5 cm) plus raw-Lout `@RightNote` / `@OuterNote` fences for Tufte-style side-notes alongside the main column. Demonstrates note attachment, automatic downward shift on collision, and the recto/verso-aware `@OuterNote` variant. [[html](out/marginalia.html) - [pdf](out/marginalia.pdf)] |
| [`multilingual.md`](multilingual.md) | doc | 3 | Three character regimes in one document: accented Western-European Latin via raw-Lout `@Char` calls, the full Greek alphabet plus math operators via the Adobe Symbol font (`@Sym alpha`, `@Sym integral`, ...), a KaTeX math display, and a Russian-language paragraph via `@Language { Russian }`. Exercises the post-ec987be Symbol glyph table. [[html](out/multilingual.html) - [pdf](out/multilingual.pdf)] |
